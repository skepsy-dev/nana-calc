...
function popUpRandomMole() {
  ...
  const moleHeads = document.querySelectorAll('.wgs__ck-vx:not(.wgs__ck-vx--whacked)');
  ...
  clickable = true;
  moleHead.classList.remove('wgs__ck-vk--hidden');
...
function hideMole(mole) {
  clickable = false;
  mole.classList.add('wgs__ck-vk--hidden');
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const moleHeads = document.querySelectorAll('.wgs__ck-vx');
  for (let moleHead of moleHeads) {
    moleHead.addEventListener('click', event => {
      ...
      event.target.classList.add('wgs__ck-vk--hidden');

      // UNCOMMENT THIS LINE OF CODE WHEN DIRECTED FOR THE BONUS
      event.target.classList.add('wgs__ck-vx--whacked');
    });
  }
});
